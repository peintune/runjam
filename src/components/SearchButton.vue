      ...
      <div v-if="open" class="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]" @click.self="open = false; query = ''; results = []">
        <div class="w-[560px] bg-white rounded-2xl border border-gray-200 shadow-xl">
          <div class="flex items-center gap-3 px-4 py-3 border-b border-gray-100">
            ...
            <input ref="inputRef" v-model="query" @input="doSearch" :placeholder="$t('search.placeholder')"
          ...
          <div class="max-h-[50vh] overflow-y-auto">
            <div v-if="searching" class="px-4 py-12 text-center text-[13px] text-gray-400">{{ $t("search.searching") }}</div>
            ...
            <template v-else-if="query && results.length > 0">
              <button v-for="r in results" :key="r.session_id + r.content.substring(0,20)"
                ...
                <div class="flex items-center gap-2 mb-1">
                  <span class="text-[11px] font-medium text-gray-400 bg-gray-100 rounded px-1.5 py-0.5">{{ r.role === 'user' ? $t('search.you') : $t('search.agent') }}</span>
                  <span class="text-[11px] text-gray-400">{{ wsStore.sessions.find(s => s.id === r.session_id)?.cliDisplayName || $t('search.session') }}</span>
                </div>
                <p class="text-[13px] text-gray-700 leading-relaxed line-clamp-3">{{ r.content }}</p>
              </button>
            </template>

            <div v-else-if="query && results.length === 0" class="px-4 py-12 text-center text-[13px] text-gray-400">{{ $t("search.noResults") }}</div>

            <template v-else>
              <p class="px-4 pt-3 pb-1 text-[11px] font-semibold text-gray-400 uppercase tracking-wider">{{ $t("search.recent") }}</p>
